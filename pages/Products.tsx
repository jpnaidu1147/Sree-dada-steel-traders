...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <div className="flex flex-col bg-white border border-steel-200 rounded-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300">
      <div className="h-64 overflow-hidden relative">
        ...
        />
        <div className="absolute top-4 right-4 bg-brand-orange text-white text-xs font-bold px-3 py-1 rounded-full uppercase">
          {product.category}
        </div>
      </div>
        ...
        <div className="mb-6 space-y-2">
          {product.features.map((feature, idx) => (
            <div key={idx} className="flex items-center text-sm text-steel-500">
              <Check size={16} className="text-brand-blue mr-2" />
              {feature}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="w-full pt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        {/* Bulk Order Banner */}
        <div className="mt-20 bg-brand-blue rounded-2xl p-8 md:p-12 text-white flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
